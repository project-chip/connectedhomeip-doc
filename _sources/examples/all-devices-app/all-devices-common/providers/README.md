...
| :----------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`AllDevicesExampleDACProvider`](https://github.com/project-chip/connectedhomeip/blob/master/examples/all-devices-app/src/credentials/DeviceAttestationCredsProvider.h) | Handles Device Attestation Credentials (DAC, PAI, CD, and DAC private key signing). In POSIX test/simulation environments, supports loading test credentials from a JSON vector file (`--dac_provider <path>`), falling back to default SDK example credentials. |
| [`AllDevicesExampleDeviceInfoProviderImpl`](https://github.com/project-chip/connectedhomeip/blob/master/examples/all-devices-app/src/platform/DeviceInfoProvider.h)                                  | Provides fixed labels, user labels, supported locales, and calendar types across dynamic endpoints in `all-devices-app`.                                                                                                                                         |
| [`AllDevicesExampleDeviceInstanceInfoProviderImpl`](https://github.com/project-chip/connectedhomeip/blob/master/examples/all-devices-app/src/platform/DeviceInstanceInfoProvider.h)                  | Wraps the underlying platform `DeviceInstanceInfoProvider` delegate and overrides Vendor ID and Product ID when custom `--vendor-id` and `--product-id` CLI options are passed.                                                                                  |
...
under:
[`credentials/development/attestation/`](https://github.com/project-chip/connectedhomeip/blob/master/credentials/development/attestation)
...
-   **Simulation / Test Harnesses (POSIX)**: The POSIX entrypoint
    ([`posix/main.cpp`](https://github.com/project-chip/connectedhomeip/blob/master/examples/all-devices-app/posix/main.cpp)) initializes
